...
def calculate_color_hist(mask, img):
	...
	The number of channel is varied because of different colour spaces.
	"""
...
def calculate_texture_hist(mask, lbp_img):
	...
	Plan to switch to Gaussian derivatives as the paper in future version.
	"""
...
def slic(image, seg_num=200):
	seg_label = segmentation.slic(image, n_segments=seg_num, sigma=1, compactness=10, convert2lab=True, start_label=0)
...
class HierarchicalGrouping(object):
	...
	def build_region_pairs(self):
		...
		for i in self.labels:
			...
			for j in neighbors:
				if i < j:
					self.s[(i, j)] = calculate_sim(self.regions[i], self.regions[j], self.img.size, self.sim_strategy)
	...
	def merge_region(self, i, j):
		...
		new_box = (min(ri['box'][0], rj['box'][0]), min(ri['box'][1], rj['box'][1]), max(ri['box'][2], rj['box'][2]), max(ri['box'][3], rj['box'][3]))
		value = {
			'box': new_box,
			'size': new_size,
	...
	def calculate_similarity_for_new_region(self):
		...
		for j in neighbors:
			...
			self.s[(j, i)] = calculate_sim(self.regions[i], self.regions[j], self.img.size, self.sim_strategy)
...
def simple_superpixel(batch_image: np.ndarray, superpixel_fn: callable) -> np.ndarray:
	...
	batch_out = Parallel(n_jobs=num_job)(delayed(superpixel_fn)(image) for image in batch_image)
